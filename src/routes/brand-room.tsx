...
// Live $500 setup fee payment link (FastPayDirect / GoHighLevel)
const SETUP_FEE_PAYMENT_URL = "https://link.shopdollhouse.co/payment-link/6a29d96671a0aa761e4641ba";

function LiveSetupModal({ open, onClose }) {
  ...
  const inputCls = "w-full rounded-xl bg-white/72 border border-[var(--gold)]/30 px-4 py-3 text-[var(--ink)] placeholder:text-[var(--ink)]/35 focus:outline-none focus:border-[var(--rose)]";
  const inputStyle = { fontFamily: FONT_DISPLAY, fontSize: "1rem" };

  return (
    <>
      <div aria-hidden="true" onClick={onClose} style={{ position: "fixed", inset: 0, zIndex: 9998, background: "rgba(30,15,11,0.45)" }} />
      <div
        role="dialog"
        aria-modal="true"
        ...
        style={{
          ...
          boxShadow: "0 32px 80px -24px rgba(30,15,11,0.40)", padding: "32px 28px", boxSizing: "border-box",
        }}
      >
        <button
          type="button"
          ...
          onClick={onClose}
          style={{
            position: "absolute", top: "16px", right: "16px", display: "inline-flex",
            width: "40px", height: "40px", alignItems: "center", justifyContent: "center",
            border: "1.5px solid rgba(189,116,118,0.5)", borderRadius: "50%",
            background: "rgba(189,116,118,0.1)", color: "#bd7476", cursor: "pointer",
          }}
        >
        ...
        <p style={{ fontFamily: FONT_LUXE, fontSize: "9px", fontWeight: 700, letterSpacing: "0.22em", textTransform: "uppercase", color: "#bd7476", margin: "0 0 12px" }}>
          Secure Your Spot
        </p>
        <h2 style={{ fontFamily: FONT_DISPLAY, fontStyle: "italic", fontWeight: 400, fontSize: "clamp(1.8rem, 6vw, 2.4rem)", lineHeight: 1.05, color: "#bd7476", margin: "0 0 10px" }}>
          Pay your $500 setup fee and let's get started.
        </h2>
        <p style={{ fontFamily: FONT_BODY, fontSize: "0.88rem", lineHeight: 1.6, color: "rgba(30,15,11,0.62)", margin: "0 0 20px" }}>
        ...
        <div className="space-y-2" style={{ marginBottom: "16px" }}>
          {planOptions.map((p) => {
            ...
            return (
              ...
              >
                <span style={{ fontFamily: FONT_DISPLAY, fontSize: "1.15rem", color: "var(--ink)" }}>{p.label}</span>
                <span style={{ display: "block", fontFamily: FONT_BODY, fontSize: "0.78rem", color: "rgba(30,15,11,0.55)" }}>{p.desc}</span>
              </button>
            );
          })}
        </div>
        ...
        <div className="space-y-3">
          <input type="text" placeholder="Your name" value={name} onChange={(e) => setName(e.target.value)} className={inputCls} style={inputStyle} />
        ...
        <p className="mt-3 text-center" style={{ fontFamily: FONT_BODY, fontSize: "0.72rem", color: "rgba(30,15,11,0.45)" }}>
